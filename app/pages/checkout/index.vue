...
const checkoutMainItemIds = computed(() => {
  const calcItems = checkoutItems.value
  if (calcItems.length > 0) {
    return normalizeRecommendationItemIds(calcItems.map(resolveRecommendationMainItemId))
  }

  return normalizeRecommendationItemIds(
    items.value.filter((item) => item.selected).map(resolveRecommendationMainItemId)
  )
})

const { products: recommendedProducts, visible: recommendationsVisible, reload: reloadRecommendations } =
  useProductRecommendations({
    scene: 'checkout',
    mainItemIds: checkoutMainItemIds,
    enabled: computed(() => checkoutInitialized.value && checkoutMainItemIds.value.length > 0),
  })
