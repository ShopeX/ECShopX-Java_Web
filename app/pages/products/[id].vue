<template>
  <div class="bg-white">
    ...
    <!-- 商品详情内容：PC 左栏滚动（图+详情），右栏购买区 sticky；推荐区在 flex 行外全宽滚动 -->
    <div v-else-if="rawProductData" class="w-full bg-white">
      <div
        class="flex w-full flex-col items-start lg:mx-auto lg:max-w-[1680px] lg:flex-row lg:items-start lg:gap-[50px] lg:px-[120px]"
      >
        <!-- 左栏：缩略图 + 主图 + 商品详情 -->
        <div class="flex w-full min-w-0 flex-1 flex-col lg:flex-row">
          <!-- Left Column: Thumbnail Navigation (PC Only) -->
          <div class="hidden lg:flex shrink-0 flex-col items-center gap-4 px-8 py-0">
        <!-- Up Arrow -->
        <button
        ...
        >
          ...
          <!-- Middle Column: Main Image (+ PC 商品详情) -->
          <div class="relative flex min-w-0 w-full flex-1 flex-col">
        <!-- 当前选中的大图（带过渡动画） -->
        <div
          ...
          <div v-else class="w-8"></div>
            ...
            <div class="hidden w-full lg:block">
              <BCProductDetailTabs
                ...
                :reviews-total="productReviewsTotal"
                :reviews-loading="productReviewsLoading"
              />
            </div>
          </div>
        </div>

        <!-- 右栏：购买信息（PC sticky，对齐购物袋/结算页） -->
        <div
          class="flex w-full shrink-0 items-center justify-center bg-white lg:sticky lg:top-[calc(var(--layout-header-height,68px)+20px)] lg:w-[517px] lg:self-start lg:px-0"
        >
          <div class="flex h-fit w-full flex-col gap-8 px-4 py-6 lg:max-w-[517px] lg:px-0 lg:py-12">
          <div class="flex w-full flex-col gap-2">
            <BCProductMarketingTags :tags="product.marketingTags" placement="inline" />

            <div class="flex w-full items-start justify-between gap-4">
              <h1
                class="min-w-0 flex-1 font-['Noto_Sans_SC'] text-xl font-normal leading-[1.2] tracking-[-0.48px] text-[#191a1d] lg:text-2xl"
              >
                {{ product.title }}
              </h1>
              <button
          ...
          <div class="hidden w-full flex-col gap-4 lg:flex">
            <button
              class="flex w-full items-center justify-center gap-2 bg-[#0f0f10] px-0 py-4 text-sm font-medium leading-5 text-white transition-colors hover:bg-[#2a2a2a] disabled:cursor-not-allowed disabled:bg-[#8f99aa] disabled:opacity-100"
            ...
            <button
              class="w-full border border-[#0f0f10] bg-white px-0 py-4 text-sm font-medium leading-5 text-[#191a1d] transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50"
      ...
      <!-- H5：购买区后再展示商品评价 / 商品详情 -->
      <div class="w-full px-4 py-6 lg:hidden">
        <BCProductDetailTabs
          instance-id="mobile"
          variant="mobile"
          :item-id="id"
    ...
    >
      ...
      <div class="flex-1 flex gap-4">
        <button
          class="flex flex-1 items-center justify-center gap-2 bg-[#0f0f10] px-0 py-4 text-sm font-medium leading-5 text-white transition-colors hover:bg-[#2a2a2a] active:bg-[#000] disabled:cursor-not-allowed disabled:bg-[#8f99aa] disabled:opacity-100"
          :disabled="isAddingToCart || !canAddToCart"
          @click="addToCart"
        >
          <UIcon v-if="isAddingToCart" name="i-heroicons-arrow-path" class="h-5 w-5 animate-spin" />
          {{ addToCartButtonText }}
        </button>
        <button
          class="flex-1 border border-[#0f0f10] bg-white px-0 py-4 text-sm font-medium leading-5 text-[#191a1d] transition-colors hover:bg-gray-50 active:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50"
          :disabled="!canAddToCart"
          @click="buyNow"
        >
          {{ $t('464b6330.5fd2f9') }}
...
import BCProductDetailTabs from '~/components/BCProductDetailTabs/BCProductDetailTabs.vue'
...
import { findMatchedSpecItem, resolveProductStock } from '~/utils/productSku'
import type { ProductSkuData } from '~/utils/productSku'
import { resolveRecommendationMainItemId } from '~/utils/normalizeRecommendationItemIds'
...
const realItemId = computed(() => {
  ...
  const matchedSpecItem = findMatchedSpecItem(
    data.spec_items,
    currentSpecIdString.value
  )
...
// 计算当前库存（与 SKU 抽屉共用 resolveProductStock）
const productSkuData = computed(() => rawProductData.value as ProductSkuData | undefined)

const currentStock = computed(() =>
  resolveProductStock(productSkuData.value, currentSpecIdString.value)
)

const allSpecsSelected = computed(() =>
  product.value.specs.every((spec) => selectedSpecs.value[spec.spec_name] !== undefined)
)

const canAddToCart = computed(() => {
  if (!rawProductData.value) return false
  if (product.value.specs.length > 0 && !allSpecsSelected.value) return false
  return Boolean(realItemId.value) && currentStock.value > 0
})
...
const addToCart = async () => {
  // 防止重复点击
  if (isAddingToCart.value || !canAddToCart.value) return
...
const recommendationDistributorId = computed(() =>
  resolveDistributorId((rawProductData.value as any)?.distributor_id)
)

const recommendationMainItemIds = computed(() => {
  if (!id) return []
...
const { products: recommendedProducts, visible: recommendationsVisible } = useProductRecommendations({
  scene: 'detail',
  mainItemIds: recommendationMainItemIds,
  distributorId: recommendationDistributorId,
  enabled: computed(() => !!id && !detailLoading.value && !!rawProductData.value),
})

const { addingProductId: recommendationAddingId, handleRecommendationAdd } =
  useRecommendationAddToCart({ mode: 'cart' })

onMounted(async () => {
  await Promise.all([syncFavoriteStatus(), loadReviews('1', '2')])
})
