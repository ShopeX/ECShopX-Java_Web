...
export interface ICheckoutItem {
  id: string
  productId: string
  /** SPU / 默认规格 ID，推荐匹配 main_item_ids 使用 */
  goodsId?: string
  productName: string
  productImage: string
  specName: string
...
export class OrderTransformer {
   ...
   * 转换结算商品项
   */
  static isCheckoutRecommendationApiItem(apiItem: any): boolean {
    if (!apiItem || typeof apiItem !== 'object') return false

    // getFreightFee 仅推荐商品行返回 is_recommend
    return apiItem.is_recommend === true || apiItem.is_recommend === 1
  }
  ...
  static toCheckoutItemModel(apiItem: any): ICheckoutItem {
    ...
    return {
      ...
      activityType: apiItem.activity_type ? String(apiItem.activity_type) : undefined,
      marketingTags: mapPromotionTags(apiItem),
      isCheckoutRecommendation: OrderTransformer.isCheckoutRecommendationApiItem(apiItem),
    }
  }
