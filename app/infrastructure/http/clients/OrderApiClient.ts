...
export interface ICalculateOrderRecommendItem {
  item_id: number
  num: number
}
...
export interface ICreateOrderRequest {
  ...
  /**
   * 结算页推荐加购商品
   * 与 getFreightFee 同格式，提交前会 JSON 序列化为字符串
   */
  recommend_item_id?: ICalculateOrderRecommendItem[]
  /** 收货人姓名（快递配送时必填） */
...
function buildOrderRequestBody<T extends { recommend_item_id?: ICalculateOrderRecommendItem[] }>(
  params: T
): Record<string, string | number> {
  const { recommend_item_id, ...rest } = params
  const body: Record<string, string | number> = {}

  Object.entries(rest).forEach(([key, value]) => {
    if (value !== undefined && value !== null) {
      body[key] = value as string | number
    }
  })

  // 推荐加购商品需序列化为 JSON 字符串：通用 http 层对数组会退化成 "[object Object]"
  if (recommend_item_id?.length) {
    body.recommend_item_id = JSON.stringify(
      recommend_item_id.map(({ item_id, num }) => ({ item_id, num }))
    )
  }
...
export class OrderApiClient {
  ...
  async calculateOrder(params: ICalculateOrderRequest, options?: any): Promise<any> {
    return this.http('/wxapp/getFreightFee', {
      method: 'POST',
      body: buildOrderRequestBody(params),
      ...options,
    } as any)
  }
