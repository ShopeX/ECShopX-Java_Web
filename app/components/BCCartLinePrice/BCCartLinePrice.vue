<template>
  <!-- 购物袋页 / 抽屉：会员价 + 橙色标签 + 非会员价 -->
  <div v-if="hasMemberPriceLayout" class="flex flex-col items-start gap-1">
    <div class="flex items-center gap-1">
      <span :class="memberPrimaryPriceClass">{{ effectivePriceDisplay }}</span>
      <span
    ...
    <span v-if="memberPriceStrikethroughDisplay" :class="memberSecondaryPriceClass">
      {{ t('c23b194b.nonMemberPrice') }} {{ memberPriceStrikethroughDisplay }}
    </span>
  </div>
...
const memberPrimaryPriceClass = computed(() => {
  if (props.variant === 'cart-page' || props.variant === 'mini-h5') {
    return "font-['Inter'] text-base font-medium leading-5 text-[#191a1d]"
  }

  return "font-['Noto_Sans_SC'] text-base font-medium leading-6 text-[#191a1d]"
})

const memberSecondaryPriceClass =
